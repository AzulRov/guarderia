...
<div class="d-flex justify-content-center align-items-start mt-4">
    <div class="card shadow" style="width: 36rem; border-radius: 12px; overflow: hidden; border: none;">
        ...
        <div class="text-white text-center py-3" style="background-color: #f57c00;">
            <h2 class="mb-0">Nuevo Registro de Comida</h2>
        </div>

        <div class="card-body p-4">
            <form action="{{ route('registrocomidas.store') }}" method="POST">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <select name="id_nino" class="form-control form-control-lg" required>
                        ...
                        @foreach($ninios as $ninio)
                            <option value="{{ $ninio->id_nino }}" {{ old('id_nino') == $ninio->id_nino ? 'selected' : '' }}>
                                {{ $ninio->id_nino }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="fecha" class="form-control form-control-lg" value="{{ old('fecha') }}" required>
                    @error('fecha')
                        <div class="text-danger mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="cantidad" class="form-control form-control-lg" value="{{ old('cantidad') }}" min="1" required>
                    @error('cantidad')
                        <div class="text-danger mt-1">{{ $message }}</div>
                    @enderror
                </div>
